...
for file in csv_list:
    ...
    model_IC = Model(name='Roitman data, drift varies with coherence',
    ...
    fit_model_IC = fit_adjust_model(sample=roitman_sample, model=model_IC, lossfunction=LossRobustLikelihood, verbose=False)
    
    display_model(fit_model_IC)
    
    with open(file+"_model_IC_output.txt", "w") as f2:
        f2.write(repr(fit_model_IC))
    f2.close()
    f.close()
