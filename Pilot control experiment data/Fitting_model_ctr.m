function [params_fit_si_ci, nll_si_ci, params_fit, nll, params_fit_best, nll_both] = Fitting_model_ctr(sbji,modi)
curr_dir = pwd;
...
params_psych_m1_all(1:size(mu_est,1),1:size(mu_est,2),1) = mu_est;
params_psych_m1_all(1:size(mu_est,1),1:size(mu_est,2),2) = sigma_est;
params_psych_m1_all(1:size(mu_est,1),1:size(mu_est,2),2) = lambda_est;
...
if ismember(modi, [1 2 4 6])
    ...
    if modi == 1
        ...
        PLB = [LB(1) log(0.001)    LB(3) -3];
        PUB = [UB(1) log(0.15)     UB(3) 6.5];
    elseif modi == 2
        LB = [NaN log(0.0001)    0.00001   0.50000 ]; % mu is taken from psych curve fits
        UB = [NaN  log(0.5)     0.99999 0.99999 ];
        PLB = [LB(1) log(0.001)   0.4 LB(4)   ];
        PUB = [UB(1)  log(0.15)   0.6 UB(4)  ];
    elseif modi == 4 | modi == 6
        LB = [NaN log(0.0001) -20  0.50000  ]; % mu is taken from psych curve fits
        UB = [NaN  log(0.5)  20  0.99999  ];
        PLB = [LB(1) log(0.001)   -5  LB(4)  ];
        PUB = [UB(1) log(0.15)   5  UB(4) ];
    end
elseif ismember(modi, [3 5])
    nvars = 5;
    if modi == 3
        LB = [NaN  log(0.0001)  0.00001  0.50000 -10 ]; % mu is taken from psych curve fits
        UB = [NaN   log(0.5)  0.99999  0.99999 10 ];
        PLB = [LB(1) log(0.001)   0.4  LB(4) -3  ];
        PUB = [UB(1) log(0.15)   0.6  UB(4) 6.5];
    elseif modi == 5
        LB = [NaN log(0.0001)  -20  0.50000 -10 ]; % mu is taken from psych curve fits
        UB = [NaN  log(0.5)   20  0.99999 10 ];
        PLB = [LB(1) log(0.001)  -5  LB(4) -3  ];
        PUB = [UB(1) log(0.15)  5  UB(4) 6.5];
    end
...
save(['fitsSctr_model_',num2str(modi),'_exp_',num2str(exp_i),'_sbji_',num2str(sbji) ,'.mat'], 'params_fit_si_ci', 'nll_si_ci','params_fit','nll', 'params_fit_best', 'nll_both',    '-mat')
end
